...
def headers(settings: TelegramChannelSettings) -> dict[str, str]:
    ...
    When ``api_bearer_token`` is set (recommended in production), it is sent
    as the Bearer token — typically a JWT minted for the service account.
    Otherwise the configured ``auth_user_id`` UUID is used (development MVP).
    """
