...
def _build_prompt(
    *,
    ...
    storage_model = (
        ...
        "FILE STORAGE (read this before acting):\n"
        "- Paths like /library/... are LOGICAL paths in Kragen (PostgreSQL + object storage), NOT a folder "
        "to create with mkdir under the Cursor --workspace. Never create a local directory tree or "
        ".gitkeep to represent /library/...; that does not store files in Kragen.\n"
        "- To download from a public URL into logical storage, the Kragen API does the download "
        "(server-side). You do not need to curl the external site from this process; you need to call "
        "the API or the kragen-files MCP so the server fetches the URL.\n"
        "- Preferred: MCP server 'kragen-files', tool 'import_url' (dest_folder_path e.g. /library/pq) "
        "when KRAGEN_TASK_TOKEN is set. Also: ensure_folder_path, upload_from_workspace. Allow MCP in "
        "Cursor if the UI asks.\n"
        "- Fallback: curl or httpx to POST /files/import on KRAGEN_API_URL with "
        "Authorization: Bearer <KRAGEN_TASK_TOKEN> and JSON "
        '{ "url", "workspace_id", "dest_folder_path" } (or parent_id+file_name per API docs).\n'
        "Env when the worker runs: KRAGEN_API_URL, KRAGEN_WORKSPACE_ID, KRAGEN_TASK_TOKEN, "
        "KRAGEN_TASK_WORKSPACE_DIR (on-disk path only for upload_from_workspace, not for /library/).\n"
        "Task token scope 'files:task' allows import, /files/folders, /files/folders/ensure, and upload.\n\n"
    )
    return (
