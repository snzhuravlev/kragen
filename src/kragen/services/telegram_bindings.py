...
def is_stale_telegram_update(*, last_update_id: int | None, incoming_update_id: int) -> bool:
    ...
    if incoming_update_id > last_update_id:
        return False
    # Skip only near-backtrack duplicates/out-of-order deliveries.
    # Large jumps usually indicate state drift and should be processed.
    return (last_update_id - incoming_update_id) <= _STALE_MAX_BACKTRACK
