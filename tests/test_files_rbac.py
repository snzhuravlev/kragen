...
async def test_upload_file_checks_workspace_before_storage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_create_file(*_args: object, **_kwargs: object) -> tuple[StorageEntry, None]:
    ...
    monkeypatch.setattr(files, "ensure_workspace_access", fake_ensure)
    monkeypatch.setattr(files.file_storage, "create_file_from_bytes", fake_create_file)
